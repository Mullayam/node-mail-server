...
class OutgoingMailHandler {
    ...
    async HandleAuthenticate(auth: SMTPServerAuthentication, session: SMTPServerSession, callback: (err: Error | null | undefined, response?: SMTPServerAuthenticationResponse | undefined) => void): Promise<void> {
        ...
        try {
            if (auth.method === "XOAUTH2") {
                return callback(new Error("XOAUTH2 method is not allowed,Expecting LOGIN authentication"));
            }
    ...
    async HandleMailFrom(address: SMTPServerAddress, session: SMTPServerSession, callback: (err?: Error | null | undefined) => void): Promise<void> {
        try {
            let message = "";
            const mailFrom = MiscellaneousHelper.extractEmail(address.address) || address.address;
    ...
    async HandleRcptTo(address: SMTPServerAddress, session: SMTPServerSession, callback: (err?: Error | null | undefined) => void): Promise<void> {
        try {
            ...
            if (session.envelope && session.envelope.mailFrom) {
                const recipientMail = MiscellaneousHelper.extractEmail(address.address) || address.address;

                const mailFrom = MiscellaneousHelper.extractEmail(session.envelope.mailFrom.address) || session.envelope.mailFrom.address;
                const mailFromDomain = mailFrom.split('@')[1];
    ...
    private generateDKIMSignature() {
        ...
        // Method 1
        const signature = spam.signDkim(headers, emailBody, "example.com", "sender.com", PVT_KEY);

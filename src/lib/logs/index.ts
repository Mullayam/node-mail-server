...
class Logger {
	...
	constructor() {
		...
		console.log(
			greenBright(
				`[${Logger.appName}] ${yellow(process.pid)} - ${white(moment().format("DD/MM/YYYY hh:mm:ss A"))}, [INFO] ${Logger.name} Service Iniatialized`,
			),
		);
	}
	setName(name: string) {
	...
	dev(text: string, type: LoggingLevel = "info") {
		if (type === "info") {
			return console.log(
				greenBright(
					`[${Logger.appName}] ${yellow(process.pid)} - ${white(moment().format("DD/MM/YYYY hh:mm:ss A"))}, [${(type).toUpperCase()}] ${text}`,
				),
			);
		}
		if (type === "error") {
			return console.log(
				red(
					`[${Logger.appName}] ${process.pid} - ${white(moment().format("DD/MM/YYYY hh:mm:ss A"))}, [${(type).toUpperCase()}] ${text}`,
				),
			);
		}
		if (type === "debug") {

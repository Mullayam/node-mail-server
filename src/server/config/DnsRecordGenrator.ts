...
export class DNSRecordGenerator {
    ...
    private formatRecords(type: DNSRecordType, name: string, data: string): RecordTypeRespose {
        return {
   ...
   async generateSPF() {
    ...
    generateDKIM(): { publicKey: string; privateKey: string; dkimRecord: any } {
        ...
        const dkimRecord = this.formatRecords("TXT", `${this.dkimSelector}._domainkey.${this.domain}`, string);
     ...
     */
    generateDMARC() {
        const DMARCRecord = this.formatRecords("TXT", `_dmarc.${this.domain}`, `v=DMARC1; p=quarantine; pct=100; rua=mailto:dmarc-reports@${this.domain}; ruf=mailto:dmarc-failures@${this.domain}`);
     ...
     */
    generateMX() {
        const mxRecord = this.formatRecords("MX", `${this.domain}`, `10 ${this.mailServer}`);
